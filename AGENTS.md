...
- **Live `graphics/` subsystem (M2.1–M2.4, ADR-0008)**: `src/lib/graphics/{contracts,renderer,scene,camera,lighting,fps}.ts`, `src/lib/graphics/effects/{manager,fog,background,grid,particles,color}.ts`, `src/lib/graphics/core/{pool,texture-cache}.ts` (M2.4: object pool + ref-counted texture cache), `src/lib/graphics/shaders/background.glsl.ts`, `src/lib/ui/effects/GraphicsBackdrop.svelte` (mounted in `AppShell.svelte`) and the FPS chip `src/lib/ui/effects/FpsMonitor.svelte`. Three.js (`three@^0.185.1`) is a dependency of `graphics/` only. The renderer exposes `subscribeFps` (windowed, allocation-free meter) and implements GFX-005 (dirty-flag render skip + `visibilitychange` pause/resume). Placeholders not yet implemented: `graphics/controls.ts` plus the empty `graphics/{objects,materials}/` directories.
