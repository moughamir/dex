...
- **Live `graphics/` subsystem (M2.1, ADR-0008)**: `src/lib/graphics/{contracts,renderer,scene,camera,lighting}.ts` and `src/lib/ui/effects/GraphicsBackdrop.svelte` (mounted in `AppShell.svelte`). Three.js (`three@^0.185.1`) is a dependency of `graphics/` only. Placeholders not yet implemented: `graphics/{particles,controls}.ts` plus the empty `graphics/{shaders,objects,materials,effects,core}/` directories.
