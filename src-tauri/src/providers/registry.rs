...
use crate::providers::{capability::Capability, error::ProviderError, provider::Provider};
...
impl ProviderRegistry {
    ...
    pub fn register(&mut self, provider: Box<dyn Provider>) -> Result<(), ProviderError> {
        ...
        if self.providers.contains_key(id) {
            return Err(ProviderError::AlreadyRegistered { id: id.to_owned() });
        }
    ...
    pub fn unregister(&mut self, id: &str) -> Result<Box<dyn Provider>, ProviderError> {
        self.providers
            .remove(id)
    ...
    #[must_use]
    pub fn providers(&self) -> Vec<&dyn Provider> {
        self.providers.values().map(Box::as_ref).collect()
    }

    /// Finds the first provider exposing the requested capability.
    #[must_use]
    pub fn capability(&self, capability: Capability) -> Option<&dyn Provider> {
        self.providers
            .values()
